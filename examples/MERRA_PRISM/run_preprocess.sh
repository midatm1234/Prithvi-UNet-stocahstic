#!/usr/bin/env bash
# Run the complete MERRA2-to-PRISM preprocessing contract.
#
...
CONFIG="${CONFIG:-$SCRIPT_DIR/MERRA_PRISM_subdomain.yaml}"
...
run_preprocess_mode() {
  ...
  if ((failed)); then
    if [[ "$mode" == "training" ]]; then
      echo "ERROR: MERRA training preprocessing failed; scalars were not recomputed." >&2
    else
      echo "ERROR: MERRA preprocessing failed for mode '$mode'." >&2
    fi
    return 1
  fi
}
...
"$PYTHON_BIN" "$SCRIPT_DIR/compute_scalars_merra_prism.py" --config "$CONFIG"
...
echo "=== MERRA-PRISM preprocessing and scalar recomputation complete ==="
