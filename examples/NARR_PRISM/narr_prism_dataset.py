    ...
    def __init__(
        ...
        if self.use_preprocessed and scalars_dir is not None:
            ...
            if selected != configured:
                raise ValueError(
                    "Strict preprocessed NARR datasets do not permit a scalar "
        ...
        self._predictor_map: Dict[Any, Dict[str, Path]] = {}
        ...
        if self.use_preprocessed:
            ...
            self._preprocessed_map = prism_preprocessed.require_daily_products(
                self._preprocessed_dir, "narr_prism", self._dates
            )
            if mode == "training":
                (
        ...
        # Raw-mode access still regrids on demand, but it no longer rebuilds a
        # scipy interpolator for every channel and tile.  One full canonical
        # target geometry is shared by all samples; apply() gathers only the
        # requested crop from those cached weights.
        self._narr_regridder = None
        if not self.use_preprocessed:
            first_predictor_var = self.predictor_vars[0][0]
            regrid_cache = narr_regrid_cache_path(self._case_preprocess_dir)
            self._narr_regridder = build_narr_regridder(
                self._predictor_map[self._dates[0]],
                first_predictor_var,
                self.fine_lat,
                self.fine_lon,
            ...
            cache_state = (
                "loaded" if self._narr_regridder.loaded_from_cache else "built"
            )
            print(
                f"[dataset] {cache_state} shared NARR barycentric geometry "
                f"from {regrid_cache}"
            )
        ...
        if scalar_dir:
            ...
            if im is not None:
                norm.assert_valid_predictor_scaler("inputs_mean", im, self.cfg)
        ...
        if self.use_preprocessed:
            ...
            for var, level in self.predictor_vars:
                arr = interpolate_narr_to_grid(
                    ...
                    self.fine_lat,
                    self.fine_lon,
                    regridder=self._narr_regridder,
                    target_slices=(lat_slice, lon_slice),
                )
                arrays.append(arr)
            stacked = np.stack(arrays, axis=0)
