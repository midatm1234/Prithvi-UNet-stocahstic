...
class CordexDownscaleDataset(Dataset):
    ...
    def __init__(
        ...
        # Predictor and target archives can use different calendars.  Preserve
        # an exact civil-date mapping instead of assuming equal integer offsets
    ...
    def _compute_time_lengths(self) -> Tuple[List[int], List[int]]:
        ...
        for predictor_path, target_path in zip(
            self.predictor_paths, self.target_paths, strict=True
        ):
    ...
    def _build_exact_target_time_index(
        ...
        """Map every predictor civil timestamp to one target timestamp."""
...
def _exact_time_index(
    ...
    *,
    ...
    def _keys(values: Sequence[object], *, role: str, path: str):
        keys = []
        indices_by_key: dict[
            Tuple[int, int, int, int, int, int, int, int], List[int]
        ] = {}
