...
#: Sentinels delimiting the generated block. Using explicit markers (rather than
#: "replace from this heading to the next rule") means hand-written interpretation
#: prose can sit immediately after the tables without being clobbered on re-render.
#: Column order for every table. Controls only *presentation*; which variants are
#: expected comes from the run's own manifest, and which are candidates rather
#: than controls comes from the scorecard.
KNOWN_ORDER = (
    "baseline",
    "spatial_ft",
    "time_only",
...
BEGIN = "<!-- RESULTS_BEGIN: generated by cordex_temporal_report.py -- do not edit by hand -->"
END = "<!-- RESULTS_END -->"
...
def render(experiment: Path) -> str:
    ...
    # Which variants the run was *supposed* to produce comes from the manifest,
    # which is written before any variant runs. Falling back to a hard-coded tuple
    # would silently drop a variant this renderer had not been taught about, and
    # an absent variant would then look like a variant that was never planned.
    expected = manifest.get("planned_variants") or [v for v in KNOWN_ORDER if v in (manifest.get("variants") or {})]
    if not expected:
        expected = list(KNOWN_ORDER)

    order = [v for v in expected if (experiment / v / "evaluation.json").is_file()]
    reports = {
    ...
    missing = [v for v in expected if v not in order]
    if missing:
        lines.append(
    ...
    for name in order:
        ...
        optimizer_steps = info.get("actual_optimizer_steps", (info.get("training_summary") or {}).get("global_step", hist[-1].get("global_step") if hist else (0 if not info.get("trained") else None)))
        val = hist[-1]["validation"]["total"] if hist else None
        lines.append(
            f"| `{name}` | {'yes' if info.get('trained') else 'no'} "
            f"| {optimizer_steps if optimizer_steps is not None else '—'} "
...
def main() -> int:
    ...
    if args.write:
        ...
        if BEGIN in text and END in text:
            ...
            raise SystemExit(
                f"{target} contains neither the {BEGIN!r} sentinel pair nor "
                f"{PLACEHOLDER!r}. Refusing to guess where the generated block goes."
            )
