"""Shared parameter helpers for the SA CORDEX fine-tune notebooks."""
...
@dataclass
class UserParams:
    """User-editable parameters shared across SA CORDEX notebooks."""
    ...
    env_run_name_var: str = "SA_RUN_NAME"
    env_inference_run_var: str = "SA_FINETUNE_RUN"
    notes: str | None = None
...
def resolve_run_dir(
    ...
    if not run_name:
        ...
        run_name = (
            str(case_name)
            if case_name
            else str(getattr(config, "job_id", "nz_finetune"))
        )
