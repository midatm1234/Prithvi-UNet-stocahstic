...
def resolve_boundary_mitigation_settings(config: Any) -> BoundaryMitigationConfig:
    ...
    halo = _to_tuple2(
        raw.get("halo", root.get("inference_halo")),
        (0, 0),
    )
    halo = (max(0, halo[0]), max(0, halo[1]))
    tile_origin = _to_tuple2(
        raw.get("tile_origin", root.get("inference_tile_origin")),
        (0, 0),
    )
...
def _compute_starts(
    ...
    # ``origin`` is diagnostic as well as functional: changing it must not
    # translate an artifact pattern. Always retain complete domain coverage by
    # inserting the physical boundary cores at zero and ``size - tile``.
...
def infer_batch_with_boundary_mitigation(
    ...
    # Reverse diffusion must evolve one global latent/noise field. Calling the
    # complete sampler independently for each tile creates unrelated priors and
    # stochastic increments, so blending the resulting samples is not a tiled
    # approximation of full-frame diffusion. Fail loudly until score evaluation
    # is tiled *inside* each global sampler step.
    ...
    halo_h = max(0, int(cfg.halo[0]))
    halo_w = max(0, int(cfg.halo[1]))
    ...
    for y0 in y_starts:
        ...
        for x0 in x_starts:
            x1 = min(x0 + tile_w, w)
            # Run the model on a context-expanded tile, then retain only the
            # central valid core. Padding-contaminated internal tile edges are
            # never accumulated into the reconstruction.
            ...
            if tuple(tile_out.shape[-2:]) != expected_core:
                raise RuntimeError(
                    "Halo tile output does not match its valid core: "
                    f"got {tuple(tile_out.shape[-2:])}, expected {expected_core}."
                )
