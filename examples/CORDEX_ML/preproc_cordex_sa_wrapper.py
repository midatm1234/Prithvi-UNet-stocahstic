#!/usr/bin/env python
"""SA-domain wrapper to run preproc_cordex.py for multiple predictor/output sets.
...
preproc_cordex.py invocation per pair. Paths are relative to SA_TEST_ROOT.
"""
...
SA_TEST_ROOT = "/mnt/data2/kyo/granite-wxc/granite-geospatial-wxc-downscaling/CORDEX/SA_domain/"

# Use paths relative to SA_TEST_ROOT to keep the lists compact.
...
TARGET_SAMPLE = (
    "/mnt/data2/kyo/granite-wxc/granite-geospatial-wxc-downscaling/CORDEX/SA_domain/"
...
def _run_preproc(predictor_files: list[str], output_dir: str) -> None:
    ...
    predictor_files = [str(Path(SA_TEST_ROOT) / p) for p in predictor_files]
    output_dir = str(Path(SA_TEST_ROOT) / output_dir)
