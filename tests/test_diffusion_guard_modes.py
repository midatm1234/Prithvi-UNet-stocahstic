...
def test_clip_mode_rescales_rms_to_limit() -> None:
    ...
    for a, e in zip(actual_rms, expected_rms, strict=True):
...
def test_validate_diffusion_vs_baseline_within_tolerance() -> None:
    ...
    assert len(caught) == len(result["warnings"])
    assert all("DEGRADATION" in str(item.message) for item in caught)
    # No failure is reported unless a channel exceeds the configured tolerance.
