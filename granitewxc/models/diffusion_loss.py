    ...
    *,
    ...
    if valid_mask is None:
        ...
        try:
            ...
            raise ValueError(
                f"valid_mask shape {tuple(valid_mask.shape)} cannot broadcast "
                f"to target shape {tuple(target.shape)}."
            ) from exc
        valid = valid & torch.isfinite(target)
    if not bool(valid.any().item()):
        raise ValueError(
            "Diffusion score matching received a batch with no finite valid "
            "target cells. Refusing an undefined zero-data update."
        )

    batch = torch.where(valid, target, torch.zeros_like(target))
    valid_f = valid.to(dtype=batch.dtype)

    def reduce_valid(values: torch.Tensor) -> torch.Tensor:
        flattened = values.reshape(values.shape[0], -1)
        flat_valid = valid_f.reshape(valid_f.shape[0], -1)
        summed = (flattened * flat_valid).sum(dim=-1)
        counts = flat_valid.sum(dim=-1)
        has_valid = counts > 0
        if reduce_mean:
            per_example = summed / counts.clamp(min=1.0)
        else:
            per_example = 0.5 * summed
        return per_example[has_valid].mean()
    ...
    # Invalid cells are neutral before the forward SDE. They contribute neither
    # random perturbations nor direct score gradients.
    ...
    epsilon_mse = reduce_valid(torch.square(epsilon_error))

    if not likelihood_weighting:
        losses = torch.square(epsilon_error)
        score_loss = reduce_valid(losses)
    else:
        g2 = sde.sde(torch.zeros_like(batch), t)[1] ** 2
        losses = torch.square(score + z / std[:, None, None, None])
        flattened = losses.reshape(losses.shape[0], -1)
        flat_valid = valid_f.reshape(valid_f.shape[0], -1)
        summed = (flattened * flat_valid).sum(dim=-1)
        counts = flat_valid.sum(dim=-1)
        has_valid = counts > 0
        per_example = (
            summed / counts.clamp(min=1.0) if reduce_mean else 0.5 * summed
        )
        score_loss = (per_example * g2)[has_valid].mean()
    ...
    if clean_weight > 0.0:
        ...
        clean_errors = torch.square(x0_prediction - batch)
        flattened = clean_errors.reshape(clean_errors.shape[0], -1)
        flat_valid = valid_f.reshape(valid_f.shape[0], -1)
        summed = (flattened * flat_valid).sum(dim=-1)
        counts = flat_valid.sum(dim=-1)
        has_valid = counts > 0
        ...
        clean_x0_loss = (stabilizer * clean_per_example)[has_valid].mean()
