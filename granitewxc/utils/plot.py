...
def plot_sample(data):
    ...
    axs[0].imshow(x[0, 1, :, :], cmap='coolwarm')
    axs[1].imshow(y[0, 0, :, :], cmap='coolwarm')
    axs[2].imshow(x[0, 2, :, :], cmap='coolwarm')
    axs[3].imshow(y[0, 1, :, :], cmap='coolwarm')
    ...
    for col, (region_name, (lon_min, lon_max, lat_min, lat_max)) in enumerate(regions.items()):
        ...
        for row, (data, rlon, rlat, title) in enumerate(datasets):
            ...
            try:
                ...
                ax.pcolormesh(cropped_rlon, cropped_rlat, cropped_data,
